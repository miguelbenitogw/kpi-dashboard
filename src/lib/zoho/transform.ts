...
export function transformCandidate(zoho: Record<string, unknown>) {
  return {
    ...
    // job_opening_id and job_opening_title will be set separately when we have association data
...
export function transformJobOpening(zoho: Record<string, unknown>) {
  ...
  return {
    id: String(zoho.id),
    title:
      (zoho.Job_Opening_Name as string) ||
      (zoho.Posting_Title as string) ||
      '',
...
// Status change detection: compare previous status with current
