...
const VALID_PHASES = ['zoho', 'madre', 'social', 'cvs', 'colocacion'] as const
type SyncPhase = (typeof VALID_PHASES)[number]
  ...
  server.tool(
    ...
    'Trigger a data synchronization phase for the KPI Dashboard. Calls the internal sync-all API endpoint.',
    {
      phase: z
        ...
        .describe(
          'The sync phase to trigger: zoho (Zoho Recruit), madre (Excel Madre sheets), social (social media), cvs (CV weekly counts), colocacion (placement data)'
        ),
    },
    async ({ phase }) => {
      const appUrl =
          ...
          content: [
            {
              type: 'text' as const,
              text: JSON.stringify(
                { error: 'SYNC_API_KEY environment variable is not configured.' },
                null,
                2
              ),
            },
      ...
      try {
        response = await fetch(
          `${appUrl}/api/admin/sync-all?phase=${phase}`,
          {
            method: 'POST',
            headers: {
  ...
  server.tool(
    ...
    'Get the most recent sync operation logs from the KPI Dashboard. Returns phase, status, inserted counts, and any errors.',
    {
      limit: z
        ...
        .default(10)
        .optional()
        .describe('Number of log entries to return (default 10, max 50)'),
    },
    async ({ limit = 10 }) => {
      ...
      const { data, error } = await mcpSupabase
        .from('sync_log_kpi')
        .select('id, created_at, phase, status, inserted, errors')
            ...
            content: [
              {
                type: 'text' as const,
                text: JSON.stringify(
                  { message: 'sync_log_kpi table not found', error: error.message },
                  null,
                  2
                ),
              },
